<!-- src/components/shared/ConcentradoReportButton.vue -->
<script setup>
import { ref, computed } from 'vue'
import { FileText, Download, FileSpreadsheet, Loader2 } from 'lucide-vue-next'
import { exportarConcentradoExcelCompleto } from '@/utils/reports/concentradoReportGenerator'
import { exportarConcentradoPDFCompleto } from '@/utils/reports/concentradoPDFGenerator'
...
const props = defineProps({
  ...
  // Datos del kanban (opcional, si está disponible)
  kanban: {
    type: Object,
    default: null
  },
...
// CRÍTICO: Solo permitir reportes cuando el estado es "vendido_a_comercializadora"
const puedeGenerarReporte = computed(() => {
  return props.concentrado?.estado === 'vendido_a_comercializadora'
})

const generarExcel = async () => {
  if (!puedeGenerarReporte.value) {
    uiStore.showWarning('El reporte solo está disponible para concentrados vendidos', 'Reporte no disponible')
    return
  }
  ...
  try {
    ...
    exportarConcentradoExcelCompleto(props.concentrado, props.kanban, props.rol)
...
const generarPDF = async () => {
  ...
  try {
    ...
    exportarConcentradoPDFCompleto(props.concentrado, props.kanban, props.rol)
...
const toggleOpciones = () => {
  if (!puedeGenerarReporte.value) {
    uiStore.showWarning(
      'Los reportes consolidados solo están disponibles para concentrados vendidos a comercializadora', 
...
const handleClickOutside = (event) => {
  if (!event.target.closest('.concentrado-report-button-container')) {
    mostrarOpciones.value = false
  }
}
...
<template>
  <div class="concentrado-report-button-container relative inline-block" :class="props.class">
    <!-- Variant: Dropdown -->
    ...
    <!-- Tooltip para concentrados no vendidos -->
    <div
    ...
    >
      Disponible solo para concentrados vendidos
...
<style scoped>
.concentrado-report-button-container {
  position: relative;
}
</style>