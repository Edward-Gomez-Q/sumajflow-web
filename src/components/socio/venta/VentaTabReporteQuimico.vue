...
const yaSubioReporte = computed(() => !!props.venta.reporteSocio)
      ...
      <div class="bg-surface rounded-xl p-5 border border-border">
        ...
        <div v-if="venta.reporteSocio" class="space-y-3">
          ...
          <div v-if="venta.reporteSocio.fechaAnalisis || venta.reporteSocio.fechaRecepcionLaboratorio" class="pt-3 border-t border-border space-y-2">
            ...
            <div v-if="venta.reporteSocio.fechaEmpaquetado && venta.tipoLiquidacion === 'venta_concentrado'" class="flex justify-between text-xs">
              <span class="text-tertiary">Empaquetado:</span>
              <span class="text-neutral">{{ formatDate(venta.reporteSocio.fechaEmpaquetado) }}</span>
            </div>
          </div>
...
<div v-if="venta.reporteSocio.urlPdf" class="pt-3">
  ...
  >
    ...
    Ver Reporte PDF
      ...
      <div class="bg-surface rounded-xl p-5 border border-border">
        <div class="flex items-center gap-3 mb-4">
          <div class="w-10 h-10 rounded-lg center shrink-0" :class="venta.reporteComercializadora ? 'bg-green-500' : 'bg-orange-500'">
            <component :is="venta.reporteComercializadora ? CheckCircle2 : Clock" class="w-5 h-5 text-white" />
          </div>
          <div class="flex-1">
            <h4 class="font-semibold text-neutral">Reporte Comercializadora</h4>
            <p class="text-xs text-secondary">{{ venta.reporteComercializadora ? 'Subido' : 'Pendiente' }}</p>
          </div>
        </div>
